...
public void setup() {
  ...
  if(device == null || !device.enabled()) device = new RainbowduinoCubeDevice(this); //delete this line (and RainbowduinoDevice.pde) if no Rainbowduino support wanted
...
public void rainbowduinoAvailable(Rainbowduino _rainbowduino) {
  ((StandaloneDevice) device).init(_rainbowduino);
  mark_for_update();
}
...
public class CubeApplet extends PApplet {
   ...
   public void setup() {
       ...
       size(640, 640, P3D);
        ...
        camera1 = new Camera( this, (float)(a/2.0f), (float)(a/2.0f), a * 2.0f, // eyeX, eyeY, eyeZ
    ...
    public void draw() {
      ...
      camera( a2,  a2, distance, // eyeX, eyeY, eyeZ
              a2,  a2,  a2,
             0.0f, 1.0f, 0.0f); // centerX, centerY, centerZ
      ...
      // Basement
      pushMatrix();
      translate( a2, a2,  PApplet.parseFloat(a) / -6.0f);
      fill(0xff666666);
      noStroke();
      box (a* 1.5f, a * 1.5f, 1.0f);
      popMatrix();
      ...
      for(int y = 0; y < f.rows; y++) {
        for(int x = 0; x < f.cols; x++) {
          ...
          int y1 = (int) (Math.ceil(Math.abs(x - 3.5f)) - 1);
          int z1 = y % 4;
    ...
    for(int y = 0; y < this.rows; y++) {
      for(int x = 0; x < this.cols; x++) {
        ...
        if(device.draw_as_circle()) {
          canvas.ellipse( draw_rad * (x + 0.5f), draw_rad * (y + 0.5f), ssize, ssize);
        }
        else {  
          canvas.rect( draw_rad * (x + 0.5f), draw_rad * (y + 0.5f), ssize, ssize);
        }  
      }
    }
    //  println( "drawn" );
...
class FrameChooser extends RectButton {
  ...
  FrameChooser(int ix, int iy, int iwidth, int iheight) {
    super(ix, iy, iwidth, iheight, 0xff111111, 0xffFFFF00);
